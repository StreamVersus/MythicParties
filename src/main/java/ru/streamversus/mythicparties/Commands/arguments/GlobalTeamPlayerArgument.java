...
public class GlobalTeamPlayerArgument extends CustomArgument<OfflinePlayer, Player> {
    public GlobalTeamPlayerArgument(String nodeName) {
        super(new PlayerArgument(nodeName), new GlobalTeamPlayerParser());
        ...
        ArgumentSuggestions<CommandSender> defaultSuggestor = ArgumentSuggestions.stringsAsync(info -> CompletableFuture.supplyAsync(() -> {
            ...
            List<String> playerList = new java.util.ArrayList<>(MythicParties.getPartyService().getParty(p).getPlayers().stream().map(OfflinePlayer::getName).toList());
...
class GlobalTeamPlayerParser implements CustomArgument.CustomArgumentInfoParser<OfflinePlayer, Player> {
    @Override
